...
$name_err = $email_err = $password_err = $confirm_password_err = $role_err = $institution_err = $workplace_err = "";
$success_message = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["role"])) {
        ...
        // Security: Ensure the submitted role is one of the allowed values
    ...
    if ($role === 'student' && empty(trim($_POST['institution']))) {
        $institution_err = "Please enter your school or institution.";
    } elseif ($role === 'chef' && empty(trim($_POST['workplace']))) {
        $workplace_err = "Please enter your restaurant or workplace.";
    }

    // Validate password
    if (empty(trim($_POST["password"]))) {
        $password_err = "Please enter a password.";
    } elseif (strlen(trim($_POST["password"])) < 8) { // Updated to 8 for consistency with JS
    ...
    // --- 2. PROCESS DATABASE OPERATION (if no validation errors) ---
    
    if (empty($name_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err) && empty($role_err) && empty($institution_err) && empty($workplace_err)) {
        ...
        try {
            ...
            if ($stmt_check->num_rows > 0) {
                ...
                // Email is unique, so proceed with inserting the new user
                $sql_insert = "INSERT INTO user (name, email, password, role, institution, workplace) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt_insert = $conn->prepare($sql_insert);

                // Hash the password for maximum security
                $param_password = password_hash($password, PASSWORD_DEFAULT);
                
                // The role now comes directly from the form
                $param_role = $role;
                
                // Bind variables to the prepared statement
                $stmt_insert->bind_param("ssssss", $name, $email, $param_password, $role, $institution, $workplace);
                
                // Attempt to execute the statement
                if ($stmt_insert->execute()) {
                    $success_message = "Registration successful! You can now <a href='login.php'>log in</a>.";
                    // Clear fields on success for a clean form
                    $name = $email = $role = ""; 
                } else {
...
<head>
    ...
    <style>
      /* These styles can be moved to your style.css file if you prefer */
...
<body>
    ...
    <div class="container">
        <div class="auth-section">
            <h1 class="auth-title">Become a Contributor</h1>

            <p style="text-align: center; margin-bottom: 2rem; color: #555;">
                Create your contributor account to share recipes, connect with the community, and showcase your skills.
            </p>
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" novalidate>
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <select name="role" class="form-input">
                        <option value="" <?php if(empty($role)) echo 'selected'; ?>>-- Please Select a Role --</option>
                        <option value="chef" <?php if($role == 'chef') echo 'selected'; ?>>Professional Chef</option>
                        <option value="student" <?php if($role == 'student') echo 'selected'; ?>>Culinary Student</option>
                    </select>
                    <span class="form-error"><?php echo $role_err; ?></span>
                </div>

                <div class="form-group" id="institution-field" style="display: none; margin-bottom: 1.5rem;">
                    <label class="form-label">School / Institution</label>
                    <input type="text" id="institution" name="institution" class="form-input" placeholder="e.g., Le Cordon Bleu, UTeM" value="<?php echo htmlspecialchars($institution); ?>">
                    <span class="form-error"><?php echo $institution_err ?? ''; ?></span>
                </div>

                <div class="form-group" id="workplace-field" style="display: none; margin-bottom: 1.5rem;">
                    <label class="form-label">Restaurant / Workplace</label>
                    <input type="text" id="workplace" name="workplace" class="form-input" placeholder="e.g., The French Laundry, Self-Employed" value="<?php echo htmlspecialchars($workplace); ?>">
                    <span class="form-error"><?php echo $workplace_err ?? ''; ?></span>
                </div>
    ...
    <script>
        ...
        if (roleSelect) {
            ...
            // Run on page load to set initial state correctly (in case of form reload)
            handleRoleChange();
        }
